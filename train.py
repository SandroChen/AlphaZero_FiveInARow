...
from mcts import BoardState, Node, mcts
...
if __name__ == '__main__':
    ...
    root = Node(white, None, BoardState(np.zeros((board_size, board_size)), white, win_condition))
    # 统计胜率
    stats = {}
    # 一个epoch走完一整盘棋
    for epoch in tqdm.tqdm(range(n_epoch)):
        ...
        node = root
        node_parents = {}
        while not node.board_state.is_terminal():
        ...
        del node_parents
        stats.setdefault(node.board_state.is_terminal(), 0)
        stats[node.board_state.is_terminal()] += 1
